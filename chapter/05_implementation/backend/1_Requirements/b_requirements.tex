\section{Requirements Engineering and Design Constraints}
...
\begin{tikzpicture}[font=\sffamily, every node/.style={align=center}]
  ...
  % y positions
  \coordinate (A) at (0,0);
  \coordinate (B) at (0,\h+\gap);
  \coordinate (C) at (0,2*\h+2*\gap);
  \coordinate (D) at (0,3*\h+3*\gap);

  % Must Have
  \node[layer, minimum width=\wA cm, minimum height=\h cm, anchor=south] (must) at (A)
    {\textbf{Must Have}\\
    \footnotesize Sub-second response time (typical load)\\
    \footnotesize High concurrency \& graceful degradation};

  % Should Have
  \node[layer, minimum width=\wB cm, minimum height=\h cm, anchor=south] (should) at (B)
    {\textbf{Should Have}\\
    \footnotesize Horizontal scalability\\
    \footnotesize Comprehensive logging \& monitoring};

  % Could Have
  \node[layer, minimum width=\wC cm, minimum height=\h cm, anchor=south] (could) at (C)
    {\textbf{Could Have}\\
    \footnotesize Advanced caching strategies\\
    \footnotesize Automated CI/CD pipeline};

  % Won't Have
  \node[layer, minimum width=\wD cm, minimum height=\h cm, anchor=south] (wont) at (D)
    {\textbf{Won't Have}\\
    \footnotesize Offline translation\\
    \footnotesize ML-based quality improvement};

  % subtle outline triangle for pyramid feel
  \draw[gray!40, line width=0.6pt]
    ($(must.south west)+(0.2,0)$) -- ($(wont.north)+(0,0.75)$) -- ($(must.south east)+(-0.2,0)$);
\end{tikzpicture}
\caption{MoSCoW prioritization for Tourlingo non-functional requirements.}
\label{fig:moscow-pyramid}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
